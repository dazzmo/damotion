...
class Bezier : public Trajectory<Scalar> {
   ...
   private:
    ...
    Scalar BernsteinPolynomial(const Scalar& t, int i, int n) {
        ...
        return math::BinomialCoefficient(n, i) * pow(1.0 - t, n - i) *
               pow(t, i);
    }
};

/**
 * @brief Truncates the Bezier curve between \f$ [t_0, t_f] \f$ at a point \f$ t \in [t_0, t_f] \f$ such that the curve over \f$ t \in [t_0, t] \f$ is returned 
 * 
 * @param curve 
 * @param t 
