...
Eigen::Ref<Eigen::MatrixXd> Program::AddParameters(const std::string &name,
                                                   int n, int m) {
...
Eigen::Ref<Eigen::MatrixXd> Program::GetParameters(const std::string &name) {
  auto it = parameters_.find(name);
  if (it == parameters_.end()) {
    throw std::runtime_error("Parameters with name " + name +
...
void Program::RemoveParameters(const std::string &name) {
  ...
  auto it = parameters_.find(name);
  if (it == parameters_.end()) {
    std::cout << "Parameters with name " << name << " is not in the program!";
  } else {
...
Binding<BoundingBoxConstraint> Program::AddBoundingBoxConstraint(
  ...
  bounding_box_constraints_.push_back(Binding<BoundingBoxConstraint>(con, {x}));
