...
void SolverBase::EvaluateCost(Cost &cost, const Eigen::VectorXd &x,
  ...
  for (int i = 0; i < nv; ++i) {
    if (continuous[i]) {
      ...
      inputs[i] =
          x.data() + GetCurrentProgram().GetDecisionVariableIndex(var[i][0]);
    } else {
      // Construct a vector for this input
      Eigen::VectorXd xi(var[i].size());
      for (int ii = 0; ii < var[i].size(); ++ii) {
        xi[ii] = x[GetCurrentProgram().GetDecisionVariableIndex(var[i][ii])];
      }
      vecs.push_back(xi);
  ...
  if (grd) {
    for (int i = 0; i < nv; ++i) {
      UpdateVectorAtVariableLocations(objective_gradient_cache_,
    ...
    for (int i = 0; i < nv; ++i) {
      for (int j = i; j < nv; ++j) {
        ...
        UpdateHessianAtVariableLocations(
            lagrangian_hes_cache_, cost.HessianFunction().getOutput(cnt),
            var[i], var[j], continuous[i], continuous[j]);
        cnt++;
      }
    }
  }
...
void SolverBase::EvaluateConstraint(Constraint &c, const int &constraint_idx,
  ...
  for (int i = 0; i < nv; ++i) {
    ...
    if (continuous[i]) {
      // Set input to the start of the vector input
      inputs[i] = x.data() + GetCurrentProgram().GetDecisionVariableIndex(v[0]);
    } else {
      // Construct a vector for this input
  ...
  if (is_continuous_x && is_continuous_y) {
    ...
    for (int ii = 0; ii < var_x.size(); ++ii) {
      for (int jj = 0; jj < var_y.size(); ++jj) {
        int i_idx = GetCurrentProgram().GetDecisionVariableIndex(var_x[ii]),
            j_idx = GetCurrentProgram().GetDecisionVariableIndex(var_y[jj]);
