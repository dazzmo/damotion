...
class QPOASESSolverInstance : public SolverBase {
 public:
  ...
  QPOASESSolverInstance(Program& prog) : SolverBase(prog) {
    ...
    lbx_ =
        Eigen::VectorXd::Zero(GetCurrentProgram().NumberOfDecisionVariables());
    ubx_ =
        Eigen::VectorXd::Zero(GetCurrentProgram().NumberOfDecisionVariables());
    ...
    H_ = Eigen::MatrixXd::Zero(GetCurrentProgram().NumberOfDecisionVariables(),
                               GetCurrentProgram().NumberOfDecisionVariables());

    g_ = Eigen::VectorXd::Zero(GetCurrentProgram().NumberOfDecisionVariables());
  ...
  qpOASES::returnValue Solve() {
    ...
    for (Binding<LinearCost>& binding :
      ...
      // Evaluate the cost
      EvaluateCost(binding.Get(), primal_solution_x_, binding.GetVariables(),
                   binding.GetParameters(), continuous, true, true, false);

      // Update the gradient
      UpdateVectorAtVariableLocations(g_, binding.Get().c(),
                                      binding.GetVariable(0), continuous[0]);
    }
    // Quadratic costs
    ...
    // Solve
    int nWSR = 1000;
    qpOASES::returnValue solver_return_val =
        qpOASES::returnValue::RET_INIT_FAILED;
    if (first_solve_) {
      solver_return_val =
